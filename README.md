## Hi there 👋

# Dimitrios Bourdalos


PhD Researcher and Mechanical Engineer at the Department of Mechanical Engineering and Aeronautics, University of Patras, Greece, and a member of the Stochastic Mechanical Systems & Automation Laboratory (SMSA Lab), specializing in the dynamic identification and prognosis of stochastic mechanical systems. Expert in applying Statistical Time Series (STS), Machine Learning (ML), and AI methods for vibration-based condition monitoring of rotating machinery. Proven ability in full-cycle research, from hands-on instrumentation and field experiments to the development and validation of innovative AI-based diagnostic units. Recognized with a HFRI Fellowship and significant experience in R&D project management and EU/national fundraising.


## Research interests
- Rotating machinery fault diagnosis & prognostics
- Gearbox vibration analysis
- Statistical time-series modeling
- Structural health monitoring (SHM)
- Signal processing for dynamic systems
- Data-driven and stochastic methods for engineering applications

## Current research themes
- Vibration-based diagnostics and prognostics for gearboxes
- Fault diagnosis under varying operating conditions
- Structural and machinery health monitoring using measured dynamic response data

## Academic links
- SMSA Lab: https://sites.google.com/g.upatras.gr/smsa-lab/people
- ORCID: https://orcid.org/0000-0003-3031-1128
- Google Scholar: https://scholar.google.com/citations?user=pDNyHB0AAAAJ&hl=el&oi=ao
- ResearchGate: https://www.researchgate.net/profile/Dimitrios-Bourdalos
- Scopus: https://www.scopus.com/authid/detail.uri?authorId=57767773900
- LinkedIn: https://www.linkedin.com/in/dimitrios-m-bourdalos-2ab9921a8

## About this profile
This GitHub profile hosts research-related repositories, datasets, reproducible code, and software tools related to vibration analysis, diagnostics, and health monitoring of mechanical systems.

## Tools
- MATLAB
- Python
- LaTeX
- Git / GitHub
